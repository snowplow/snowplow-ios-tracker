...
1. Read this file. If the change is contained to one Source module (Core, Events, Network, Configuration, Storage, etc.), match that module's existing patterns before editing.
...
- `swift build` — package builds
- `swift test` — tests pass
- (CI also runs `xcodebuild` on multiple platforms; trust the CI signal for platform-specific failures)

PR shape (matches this repo's `CONTRIBUTING.md`):

- **Branch**: descriptive name (e.g. `fix/event-payload-clamping`).
